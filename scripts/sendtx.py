...
# Constants
NETWORK_URL = "https://testnet-rpc.iopn.tech"
CHAIN_ID = 984
EXPLORER_URL = "https://testnet.iopn.tech/tx/0x"
...
CONFIG = {
    ...
    "MAX_CONCURRENCY": 5,
    "MAX_RETRIES": 3,
    "MINIMUM_BALANCE": 0.001  # OPN
}

# Từ vựng song ngữ
LANG = {
    'vi': {
        'title': '✨ GỬI GIAO DỊCH - IOPN TESTNET ✨',
        'info': 'ℹ Thông tin',
        'found': 'Tìm thấy',
        'wallets': 'ví',
        ...
        'enter_amount': '✦ NHẬP SỐ LƯỢNG OPN',
        'amount_prompt': 'Số lượng OPN (mặc định 0.000001, tối đa 999): ',
        'amount_unit': 'OPN',
        ...
        'insufficient_balance': 'Số dư không đủ (cần ít nhất {required:.6f} OPN cho giao dịch)',
        ...
        'connect_success': '✅ Thành công: Đã kết nối mạng IOPN Testnet',
    ...
    'en': {
        'title': '✨ SEND TRANSACTION - IOPN TESTNET ✨',
        'info': 'ℹ Info',
        'found': 'Found',
        'wallets': 'wallets',
        ...
        'enter_amount': '✦ ENTER AMOUNT OF OPN',
        'amount_prompt': 'Amount of OPN (default 0.000001, max 999): ',
        'amount_unit': 'OPN',
        ...
        'insufficient_balance': 'Insufficient balance (need at least {required:.6f} OPN for transaction)',
        ...
        'connect_success': '✅ Success: Connected to IOPN Testnet',
    ...
    for attempt in range(CONFIG['MAX_RETRIES']):
        try:
            ...
            if balance < required_balance:
                print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance'].format(required=required_balance)}: {balance:.6f} OPN{Style.RESET_ALL}")
                return False
...
async def process_wallet(index: int, profile_num: int, private_key: str, proxy: str, w3: Web3, choice: str, tx_count: int, amount: float, addresses: list, language: str):
    ...
    if eth_balance < CONFIG['MINIMUM_BALANCE']:
        print(f"{Fore.RED}  ✖ {LANG[language]['insufficient_balance'].format(required=CONFIG['MINIMUM_BALANCE'])}: {eth_balance:.6f} OPN{Style.RESET_ALL}")
        return 0
